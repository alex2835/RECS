...
int main( void )
{
...

    {
        ...
        count = 0;
        registry.RuntimeForEach( { Speed::Name() }, [&]( recs::Entity entity )
        {
            count++;
        } );
        assert( count == 20 );

        auto runtimeView = registry.GetRuntimeView( { Speed::Name() } );
        assert( runtimeView.Size() == 20 );

        runtimeView = registry.GetRuntimeView( { Speed::Name(), Position::Name() } );
        assert( runtimeView.Size() == 10 );
